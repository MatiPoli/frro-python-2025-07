...
# Define el alcance necesario para acceder a las suscripciones y detalles del canal
...
def profile_view(request):
    ...
    if not youtube_service:
        # Simula la redirección a una página de login si el usuario no está autenticado
        return HttpResponseRedirect(reverse('home')) # Asume que hay una vista 'home'
    
    try:
        # Obtiene la lista de suscripciones del usuario y los detalles de los canales
        subscriptions_list = get_subscriptions_with_details(youtube_service)
